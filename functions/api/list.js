function okSite(req,env){
return (req.headers.get('X-Site-Password')||'')===(env.SITE_PASSWORD||'')
}

export async function onRequestGet(context){
if(!okSite(context.request,context.env)) return new Response(JSON.stringify({error:'Unauthorized'}),{status:401,headers:{'Content-Type':'application/json','Cache-Control':'no-store'}})
try{
